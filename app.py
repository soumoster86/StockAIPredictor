...
with st.sidebar:
    ...
    with st.expander("ℹ️ How this app works"):
        st.markdown(
            "An ensemble of models — a neural network, random forest and "
            "XGBoost — learns patterns in price, momentum, volatility, volume "
            "and the stock's strength relative to the NIFTY, then estimates the "
            "probability of a meaningful up move over 1 to 20 days. Where a "
            "pooled **global model** (trained on the whole watchlist) is "
            "available, it's used automatically. Every signal is tested on past "
            "data the model never saw — with walk-forward validation, "
            "probability calibration, realistic trading costs, and a "
            "random-signal benchmark that checks the strategy actually beats "
            "luck. **Educational tool — not investment advice.**"
        )
...
with tab_plan:
    ...
    rr = plan['reward_risk']
    # A very high R:R usually means a noise-tight stop, not a great trade —
    # flag it rather than rewarding it. Healthy band is roughly 1.5–6.
    rr_note = ("⚠️" if rr >= 8 else "✅" if rr >= 2 else
               "⚠️" if rr >= 1.5 else "❌")
    rr_suffix = " (stop may be too tight)" if rr >= 8 else ""
    t4.metric("Reward : Risk", f"{rr_note} 1 : {rr:.1f}{rr_suffix}",
              help=HELP["reward_risk"])
